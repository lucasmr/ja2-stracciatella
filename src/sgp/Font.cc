...
INT16 StringPixLength(const ST::string& str, SGPFont font)
{
	...
	return StringPixLength(codepoints, font);
}
...
{
	ST::utf32_buffer codepoints = str.to_utf32();
	FindFontRightCoordinates(sLeft, sTop, sWidth, sHeight, codepoints, font, psNewX, psNewY);
}
...
{
	ST::utf32_buffer codepoints = str.to_utf32();
	FindFontCenterCoordinates(sLeft, sTop, sWidth, sHeight, codepoints, font, psNewX, psNewY);
}
...
{
	ST::utf32_buffer codepoints = str.to_utf32();
	MPrintBuffer(pDestBuf, uiDestPitchBYTES, x, y, codepoints);
}
